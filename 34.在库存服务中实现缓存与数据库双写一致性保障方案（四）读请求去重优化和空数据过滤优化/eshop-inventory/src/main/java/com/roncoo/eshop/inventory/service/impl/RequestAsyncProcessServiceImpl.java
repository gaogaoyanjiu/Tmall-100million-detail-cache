...
public class RequestAsyncProcessServiceImpl implements RequestAsyncProcessService {
	...
	private ArrayBlockingQueue<Request> getRoutingQueue(Integer productId) {
		...
		System.out.println("=============hash运算： "+(k = key.hashCode()) + "=============");
		System.out.println("=============hash运算： "+Integer.toBinaryString(k) + "=============");
		System.out.println("=============hash运算： "+(Integer.toBinaryString(k >>> 16)) + "=============");
...
//		100011111110101
		...
		int hash = (key == null) ? 0 : (h = key.hashCode()) ^ (h >>> 16); //无符号右移，忽略符号位，空位都以0补齐,value 移动的位数。异或（^）运算,同为假，异为真
		System.out.println("=============hash运算： "+Integer.toBinaryString(hash) + "=============");
		
		//int是4个字节，32位，h >>> 16 取出低16位
		
		System.out.println("=============内存队列长度取决于线程创建时循环的次数，循环多少次，就有多少个内存队列数量： "+ requestQueue.queueSize());
